...
import { NodeComputerRoomElement } from '../dag-canvas/elements/node-computer-room-element'
import { NodeMachineElement } from '../dag-canvas/elements/node-machine-element'
import { NodeColonyElement } from '../dag-canvas/elements/node-colony-element'
    ...
    super({
      ...
      embedding: {
        ...
        validate({ child, parent }) {
          ...
          /**
           * 1、机房与机房、集群与集群、机器与机器不能成父子关系
           * 2、
           */
          if (child.shape === parent.shape) {
            // message.warning('相同类型不能相互嵌套')
            return false
          }
          return true
        },
    ...
    if (nodeComponent === NODECOMPONENT_COLONY) {
      const node = this.graph!.addNode(
        new X6DemoNode({
          ...nodeMeta,
          shape: 'colony-rect',
          component: <NodeColonyElement experimentId={experimentId} />,
        }),
      ) as BaseNode
      if ((nodeMeta.data as any).hide) {
        this.pendingNodes.push(node)
    ...
    if (nodeComponent === NODECOMPONENT_MACHINE) {
      const node = this.graph!.addNode(
        new X6DemoNode({
          ...nodeMeta,
          shape: 'machine-rect',
          component: <NodeMachineElement experimentId={experimentId} />,
        }),
      ) as BaseNode
      if ((nodeMeta.data as any).hide) {
        this.pendingNodes.push(node)
