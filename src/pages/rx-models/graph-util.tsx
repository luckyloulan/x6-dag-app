...
// 格式化单个节点，新增节点时复用 ---- 将业务数据导入图Node数据（NodeMate）中
export function formatNodeInfoToNodeMeta(
  // 经 algo 节点数据与 graph.ts 的 addNode(此为自定的方法，非图方法)合并之后的数据
  nodeData: NExperimentGraph.Node,
  // *暂时不要生成连接桩 ---- 2021年7月24日 21:13:51
  inputPortConnectedMap: { [k: string]: boolean } = {},
) {
  const portItems: any[] = []
  const { id, nodeComponent, nodeInstanceId, positionX, positionY } = nodeData
  ...
  // TODO 暂时没有定义这玩意儿的地方，带考虑定义在哪 ----wangjiangui----2021年7月24日 21:44:49
  return {
    ...nodeData,
    x,
    y,
    // !这玩意在 experiment-graph.tsx-renderNode()方法中使用，根据拖拽的节点来判断需要展示的ReactComponent
